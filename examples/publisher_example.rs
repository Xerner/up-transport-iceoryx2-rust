...
/// Publishes messages to a given topic using the Iceoryx2 transport.
#[derive(Parser)]
...
struct Command {
    ...
    topic: UUri,
    #[command(flatten)]
    transport_options: Iceoryx2TransportOptions,
}
...
async fn main() -> Result<(), UStatus> {
    ...
    let client = Iceoryx2Transport::new(command.transport_options, authority).await?;
...
async fn connect(client: &Iceoryx2Transport) -> Result<(), UStatus> {
    let connect_wrapper_fn = || {

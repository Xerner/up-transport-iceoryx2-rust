...
use up_rust::{UCode, UListener, UMessage, UMessageType, UStatus, UTransport, UUri};
...
#[async_trait]
impl UTransport for UTransportIceoryx2 {
    async fn send(&self, _message: UMessage) -> Result<(), UStatus> {
        let _message_type = _message.type_();
        todo!();
    }
...
mod tests {
    ...
    fn test_publish_service_name() {
        ...
        let name = UTransportIceoryx2::compute_service_name(&source, None).unwrap();
    ...
    fn test_notification_service_name() {
        ...
        let name = UTransportIceoryx2::compute_service_name(&source, Some(&sink)).unwrap();
    ...
    fn test_rpc_request_service_name() {
        ...
        let name = UTransportIceoryx2::compute_service_name(&sink, Some(&reply_to)).unwrap();
    ...
    fn test_missing_uri_error() {
        let uuri = UUri::new();
        let result = UTransportIceoryx2::compute_service_name(&uuri, None);
    ...
    fn test_fail_resource_id_error() {
        ...
        let sink = test_uri("device1", 0x0004, 0x3AB, 0x3, 0x0000);
        let result = UTransportIceoryx2::compute_service_name(&source, Some(&sink));
    ...
    fn test_fail_missing_sink_error() {
        let source = test_uri("device1", 0x0000, 0x00CD, 0x04, 0x000);
        let result = UTransportIceoryx2::compute_service_name(&source, None);
        assert!(result.is_err_and(|err| err.get_code() == UCode::INVALID_ARGUMENT));
    }
    ...
    fn test_fail_missing_source_error() {
        ...
        let result = UTransportIceoryx2::compute_service_name(&uuri, Some(&sink));
        assert!(result.is_err_and(|err| err.get_code() == UCode::INVALID_ARGUMENT));
    }
}
